...
int main(int argc, char *argv[])
{
    ...
    string s4 = "../../RESULTS/QT/Convergence_s8J105";
    s4.append(argv[1]);
    s4.append(".txt");
    ...
    cx_mat Heff = H + arma::cx_double(0., 1.)*K;
    cx_mat E = arma::expmat(-arma::cx_double(0.,1.)*Heff*dt);
    ...
    {   
        ...
        {
            //cout << "n = " << n << endl;
            dpUp = real(((arma::trans(Cup*phi0))*(Cup*phi0)).eval().at(0,0))*dt;
            dpDown = real(((arma::trans(Cdown*phi0))*(Cdown*phi0)).eval().at(0,0))*dt;
            ...
            //if(IsFree)
            //myfileConv << n << '\t' << real((arma::trans(phi0)*sigmaZ[0]*phi0).eval().at(0,0)) << endl;
        }
        ...
        corrFunc4.push_back((arma::trans(PHI.back())*sigmaZ[0]*sigmaZ[4]*PHI.back()).eval().at(0,0));
        corrFunc04->Fill(real(corrFunc4.back()));

        corrFunc5.push_back((arma::trans(PHI.back())*sigmaZ[0]*sigmaZ[5]*PHI.back()).eval().at(0,0));
        corrFunc05->Fill(real(corrFunc5.back()));

        corrFunc6.push_back((arma::trans(PHI.back())*sigmaZ[0]*sigmaZ[6]*PHI.back()).eval().at(0,0));
        corrFunc06->Fill(real(corrFunc6.back()));

        corrFunc7.push_back((arma::trans(PHI.back())*sigmaZ[0]*sigmaZ[7]*PHI.back()).eval().at(0,0));
        ...
        spinCurr3.push_back((arma::trans(PHI.back())*(sigmaX[3]*sigmaY[4]-sigmaY[3]*sigmaX[4])*PHI.back()).eval().at(0,0));
        spinCurr34->Fill(real(spinCurr3.back()));

        spinCurr4.push_back((arma::trans(PHI.back())*(sigmaX[4]*sigmaY[5]-sigmaY[4]*sigmaX[5])*PHI.back()).eval().at(0,0));
        spinCurr45->Fill(real(spinCurr4.back()));

        spinCurr5.push_back((arma::trans(PHI.back())*(sigmaX[5]*sigmaY[6]-sigmaY[5]*sigmaX[6])*PHI.back()).eval().at(0,0));
        spinCurr56->Fill(real(spinCurr5.back()));

        spinCurr6.push_back((arma::trans(PHI.back())*(sigmaX[6]*sigmaY[7]-sigmaY[6]*sigmaX[7])*PHI.back()).eval().at(0,0));
    ...
    corrFunc04->Draw();
    can52->Update();
    TCanvas* can62 = new TCanvas;
    corrFunc05->Draw();
    can62->Update();
    TCanvas* can72 = new TCanvas;
    corrFunc06->Draw();
    can72->Update();
    TCanvas* can82 = new TCanvas;
    corrFunc07->Draw();
    can82->Update();
    ...
    realZ4->Draw();
    can53->Update();
    TCanvas* can63 = new TCanvas;
    realZ5->Draw();
    can63->Update();
    TCanvas* can73 = new TCanvas;
    realZ6->Draw();
    can73->Update();
    TCanvas* can83 = new TCanvas;
    realZ7->Draw();
    can83->Update();
